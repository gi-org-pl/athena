...
function Badge({
  ...
  const iconClassName = "[color:inherit] shrink-0 flex items-center justify-center";
  ...
  const renderedIcon = React.useMemo(() => {
    ...
    if (type !== "default") {
      ...
      return Icon ? <span className={iconClassName}><Icon /></span> : null;
    }
    return null;
  }, [LeftIcon, type]);
      ...
      {isDismissible && (
        <button
          ...
          className={cn(
            ...
            "text-[#D3D9DA] hover:brightness-90", // 10% darker via brightness(90%)
            size === "small" ? "size-4" : size === "big" ? "size-6" : "size-5"
          )}
        >
          <CustomDismissIcon 
            className={cn(
              size === "small" ? "size-3.5" : size === "big" ? "size-5" : "size-4.5",
              "cursor-pointer"
            )} 
          />
        </button>
