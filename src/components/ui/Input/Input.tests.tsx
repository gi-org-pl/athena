...
describe("Input Component - 100% Coverage Suite", () => {
  ...
  it("renders label and required asterisk correctly", () => {
    render(<Input label="Label" isRequired />);
...
it("handles disabled state correctly", () => {
  ...
  it("handles isRequired without label", () => {
    render(<Input isRequired dataTestId="req-no-label" />);
    expect(screen.queryByText("*")).not.toBeInTheDocument();
  });

  it("shows suffix and prefix only when hasValue is true", () => {
  ...
  it("renders secondary text when isDisabled is false", () => {
  ...
  it("handles empty displayValue and safeValue correctly", () => {
    render(
  ...
  it("does not update internal value via setInternalValue on change when controlled", () => {
    const onChange = vi.fn();
  ...
  it("shows overlay content for normal text", () => {
  ...
  it("handles null value to cover fallback string logic", () => {
    render(<Input value={null as unknown as string} />);
    const input = screen.getByRole("textbox");
    expect(input).toHaveValue("");
  });

  it("applies error and disabled styles to label", () => {
  ...
  it("renders RightIcon with disabled styles", () => {
    render(
